...
def first_game_start():
    ...
    while True:
        count_1 = input("원하는 랜덤생성 숫자 자리수를 입력하세요(3자리이하 양의 수만 가능) : ")
        if count_1.isdigit():
...
def number_finding(count_1, count_2):
    ...
    print()
    print(f"입력하신 랜덤생성 숫자는 {count_1}자리수이고 그에따라, {count_1}자리수의 양의 정수의 랜덤 숫자가 생성되었습니다.")
    print()
        ...
        while True:
            if a.isdigit():
                ...
                if int(a) == rand_num:
                    ...
                    elif 0 <= int(a) < rand_num:
                        b = smaller_than_rand_number(a, rand_num, trial, count_2)
                        break

            else:
                print(f"입력하신 값은 {a}로, 유효하지 않은 값입니다. \n\n다시 입력해 주세요. \n")
                break
